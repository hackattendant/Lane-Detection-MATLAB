...
function [X, lines_left, lines_right] = pipeline(frame, old_left, old_right)
    close all;
    ...
    xy1 = []; xy2 = [];
    %% Threshold
    threshed_img = threshold(frame);
    %% Mask Image
    dimens = size(threshed_img);
    m = dimens(1);
    n = dimens(2);
    ...
    %% draw polygon and calculate estimated offset from center
